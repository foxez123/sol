/**
 * Web3로 블록체인 데이터 변경 시 리턴값을 받을 수 없어서
 * 급히 id 가져오는 부분을 수정함
 */
...
contract CertificateManager {
    ...
    // address: certTypeId
    mapping (address => CertType) certTypes; 
    struct CertType {
        address certTypeId;
        string name;
        string issuerName;
    ...
    // address: certId
    mapping (address => PersonalCert) personalCerts;
    struct PersonalCert {
        address certId;
    ...
    function generateCertTypeId() constant public returns(address) {
        return UniqueId.getUniqueId(1);   // 1 = certTypeId
    }
    
    function generateCertId() constant public returns(address) {
        return UniqueId.getUniqueId(2);   // 2 = certId
    }
    
    function generateCertToken() constant public returns(address) {
        return UniqueId.getUniqueId(3);   // 3 = certToken
    }
    
    /*
    function getUniqueId(string _userId, uint _random) private view returns (address) {
        bytes20 b = bytes20(keccak256(msg.sender, _userId, _random, now));  // generate by time..
        uint addr = 0;
    ...
    */ 
    function registerCertType(address certTypeId, string _name, string _issuerName, string _descUrl) public {
        require( bytes(_name).length != 0);
        
        CertType memory newCertType;
        newCertType.certTypeId = certTypeId;
        ...
        certTypes[certTypeId] = newCertType;
        //certTypeIdGenerator++;
    ...
    */
    function viewCertTypeInfo(address _certTypeId) public view returns(string, string, string) {
    ...
    * sample: "2018-01-01", "2022-01-01", "MyCert1", 1
    * sample: "2018-02-01", "2099-12-31", "MyCert2", 2
    */
    function regiesterPersonalCert(address _certId, string _issueDt, string _validThroughDt, string _certNo, address _certTypeId) public  {
    ...
    * because of gas, 
    */
    function confirmCert(address _certId) public {
    ...
    */
    function generateCertToken(address _tokenId, address _certId, uint _validDay) public {
    ...
    */
    function viewPersonalCert(address _tokenAddress) checkTokenAddress(_tokenAddress) public view returns(string, string, string, string, string, string) {
        address certId = certTokens[_tokenAddress].certId;
        address certTypeId = personalCerts[certId].certTypeId;
    ...
    */
    function viewPersonalCertByAdmin(address _certId) public view returns(string, string, string, string, string, string, bool) {
    ...
    */
    function viewTokenDetail(address _tokenAddress) public view returns(address, uint, uint) {
        return (
...
library UniqueId {
    ...
    */  
    function getUniqueId(int _random) public view returns (address) 
    {
